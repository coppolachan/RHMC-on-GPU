...
__global__ void DoeKernelChem(float2  *in,
			      float2  *out,
			      int *tables, 
			      int *phases,
			      size_t gauge_offset,
...
void CuDoe(float2 *out, 
  ...
  cudaCheckError(AT,"DoeKernel"); 
    ...
    {
      DoeKernelChem<<<GridDimension,BlockDimension>>>(in, out, device_table, device_phases, offset_g, offset_f); 
      cudaCheckError(AT,"DoeKernelChem"); 
    }
