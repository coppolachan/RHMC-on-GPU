...
double AlgRemez::generateApprox(int degree, unsigned long pnum1, unsigned long pden1)
{
  return generateApprox(degree, degree, pnum1, pden1);
}


double AlgRemez::generateApprox(int num_degree, int den_degree, unsigned long pnum1, unsigned long pden1)
{
  double *a_param = 0;
  int *a_pow = 0;
  return generateApprox(num_degree, den_degree, pnum1, pden1, 0, a_param, a_pow);
}


// Generate the rational approximation x^(pnum/pden)
double AlgRemez::generateApprox(int num_degree, int den_degree,
				unsigned long pnum1, unsigned long pden1,
...
bigfloat AlgRemez::func(const bigfloat x) 
{
  ...
  bigfloat z_flav_1;
  if( (int)power_num >= 0) z_flav_1 = (bigfloat)power_num / (bigfloat)power_den;
  else z_flav_1 = -((bigfloat)(-power_num) / (bigfloat)power_den);

  bigfloat y_flav_1;
  bigfloat y;

  if(power_num == (bigfloat) 0.0 )
    {
